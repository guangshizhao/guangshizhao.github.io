            ...
            function fetchModel(modelId) {
                ...
                $http({method: 'GET', url: modelUrl}).
                    success(function (data, status, headers, config) {
                        ...
                        if(modelId){
                            var model_json = JSON.parse(result.jsonFile);
                            result.modelId = result.id;
                            result.name = result.processName;
                            result.model = model_json;
                        }else{
                            result  ={};
                            result.model = {};
                            result.model.stencilset = {
                                url:"stencilsets/bpmn2.0/bpmn2.0.json",
                                namespace:"http://b3mn.org/stencilset/bpmn2.0#"
                            }
                        }
                        $rootScope.editor = new ORYX.Editor(result);
                        $rootScope.modelData = angular.fromJson(result);
