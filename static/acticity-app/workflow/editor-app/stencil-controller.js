...
angular.module('activitiModeler')
    .controller('StencilController', ['$rootScope', '$scope', '$http', '$modal', '$timeout', function ($rootScope, $scope, $http, $modal, $timeout) {
        ...
        $scope.saveProcess = function () {
            $scope.editor.getCanvas().focus()
            $scope.editor.setSelection([]);
            jQuery("#propertiesHelpWrapper").modal('show');
        };
                ...
                if (shapes && shapes.length > 0) {
                    ...
                    for (var i = 0; i < properties.length; i++) {
                        ...
                        if (propertyConfig === undefined || propertyConfig === null) {
                            ...
                            if(key=='oryx-servicetaskdelegateexpression'){//额外添加逻辑，服务类中委托表达式不显示，并且附上默认值
                                selectedShape.properties[key] = '${jrsfServiceDelegate}';
                                continue;
                            }
                            ...
                            if(currentProperty.value && currentProperty.value.assignment){
                                
                                var assignee = currentProperty.value.assignment.assignee;
                                var candidateUsers = currentProperty.value.assignment.candidateUsers;
                                //currentProperty.value.inputshow = "1";
                                if (assignee.length == 0 && candidateUsers.length == 0) {
                                    currentProperty.value.inputshow = "";
                                  }else{
                                    if (assignee.indexOf('initiator') > -1) {
                                        //代表选中流程发起人
                                        if (assignee.indexOf('assignment.getDirectLeader') > -1) {
                                            currentProperty.value.inputshow = '流程发起人 直属领导'
                                        } else {
                                            currentProperty.value.inputshow = '流程发起人'
                                        }
                                      } else if (
                                        ...
                                        if (candidateUsers.indexOf('assignment.getDepartmentMembers') > -1) {
                                          depttype = 1
                                          deptid = candidateUsers.substring(
                                            candidateUsers.indexOf('("') + 2,
                                            candidateUsers.indexOf('")')
                                          )
                                        } else {
                                          depttype = 2
                                          deptid = assignee.substring(
                                            assignee.indexOf('("') + 2,
                                            assignee.indexOf('")')
                                          )
                                        }
                                        jQuery.ajax(
                                            {
                                                url:ACTIVITI.CONFIG.contextApi +'/Organization?condition=&pageIndex=-1&pageSize=-1',
                                                headers: {
                                              ...
                                              for(var k=0;k<result.length;k++){
                                                  if(deptid==result[k].id){
                                                    deptname = result[k].name;
                                                      break;
                                                  }
                                              }
                                             if(depttype==1){
                                                currentProperty.value.inputshow = deptname+ ' 直属成员';
                                             }else{
                                                currentProperty.value.inputshow = deptname+ ' 部门领导';
                                             }
                                            }
                                        });
                                      } else if (candidateUsers.indexOf('assignment.getRoleUsers') > -1) {
                                        //角色
                                        var roleid = candidateUsers.substring(
                                          candidateUsers.indexOf('("') + 2,
                                          candidateUsers.indexOf('")')
                                        )
                                        var rolename = "";
                                        jQuery.ajax(
                                            {
                                                url:ACTIVITI.CONFIG.contextApi + '/Role?condition=&pageIndex=-1&pageSize=-1',
                                                headers: {
                                                    "Authorization": getQueryString("token")
                                                },
                                                async:false,
                                                success:function(data){
                                                    var roles = data.data.rows;
                                                    for(var k=0;k<roles.length;k++){
                                                    if(roleid==roles[k].id){
                                                        rolename = roles[k].rname;
                                                        break;
                                                    }
                                                }
                                                currentProperty.value.inputshow = rolename;
                                        }
                                    });
                                      } else {
                                        //指定用户
                                        jQuery.ajax(
                                            {
                                                url:ACTIVITI.CONFIG.contextApi + '/User?condition=&pageIndex=-1&pageSize=-1',
                                                headers: {
                                                    "Authorization": getQueryString("token")
                                                },
                                                async:false,
                                        ...
                                        if (assignee.indexOf('assignment.getDirectLeader') > -1) {
                                          ...
                                          userid = assignee
                                                    ...
                                                    if(userid==users[k].jobNumber){
                                                        username = users[k].uname;
                                                        break;
                                                    }
                                                }
                                                if(usertype==1){
                                                    currentProperty.value.inputshow = username+ ' 直属领导';
                                                 }else{
                                                    currentProperty.value.inputshow = username;
                                                 }
                                        }
                                    });
                                  }
