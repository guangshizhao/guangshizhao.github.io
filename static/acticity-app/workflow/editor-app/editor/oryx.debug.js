...
ORYX.Editor = {
	...
	_generateGUI: function() {
		...
		this.getCanvas().setSize({
			width	: jQuery("#canvasSection").width(),
			height	: jQuery("#paletteHelpWrapper").height()
		});		
...
ORYX.Plugins.RenameShapes = Clazz.extend({
	...
	actOnDBLClick: function(evt, shape){
		...
		if(shape.properties && shape.properties["oryx-process_id"]!=undefined){//oryx-name点击空白处不弹流程信息框
			return;
		}
		if(shape._stencil && shape._stencil._jsonStencil && shape._stencil._jsonStencil.propertyPackages && shape._stencil._jsonStencil.propertyPackages.length>0){//点击开始节点弹流程信息框
			if(shape._stencil._jsonStencil.propertyPackages[shape._stencil._jsonStencil.propertyPackages.length-1] == 'startEvent'){
                this.facade.setSelection([]);
			}
		}
